    ...
    <template #header>
        <div class="flex justify-between">
            ...
            <div class="text-right">
                <a :href="route('tool.element')" class="bg-red-500 text-white rounded py-2 px-3">Back</a>
            </div>
        </div>
    </template>
...
export default {
    ...
    methods: {
        saveTool() {
            axios.post(route('tool.element.store'), this.formTool)
                .then(response => {
                    console.log(response.data)
                    this.reset()
                    window.location.href = route('tool.element')
                }).catch(error => {
                    console.log(error)
