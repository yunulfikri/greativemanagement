<template>
    <app-layout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Tool - Crella Tag</h2>
        </template>
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-3 min-h-screen">
                    <div class="flex justify-between">
                        <a :href="route('tool.crella.add')"
                                        ...
                                        <tbody>
                                            <tr v-for="(data, index) in tools" :key="data.id">
                                                ...
                                                <td class="whitespace-nowrap text-center">
                                                    <a :href="route('tool.crella.edit', data.id)" class="bg-yellow-400 text-black shadow px-2 py-1 rounded">
                                                        edit
                                                    </a>
                                                </td>
                                            </tr>
        ...
        watch:{
            search: function(val){
                if (val == '') {
                    ...
                    axios.get(route('tool.crella.search', this.search))
                    .then(response => {
